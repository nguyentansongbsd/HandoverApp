...
namespace HandoverApp.Views
{
    public partial class LoginByUserCRMPage : ContentPage
    {
        ...
        {
            await Task.Delay(1);
            loginWeb.Source = string.Format(loginUrl,OrgConfig.TeantId,OrgConfig.ClientId_ForUserCRM,OrgConfig.Redirect_Uri,OrgConfig.Scope);
            LoadingHelper.Hide();
        }

        private async void loginWeb_Navigating(System.Object sender, Xamarin.Forms.WebNavigatingEventArgs e)
        {
            if (e.Url.StartsWith(OrgConfig.Redirect_Uri))
            {
                LoadingHelper.Show();
