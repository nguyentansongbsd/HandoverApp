    ...
    public class ContactDetailPageViewModel : BaseViewModel
    {
        ...
        public async Task GetImageCMND()
        {
            if (this.singleContact.contactid != Guid.Empty)
            {
                ...
                string fileListUrl = $"https://graph.microsoft.com/v1.0/drives/{OrgConfig.Graph_ContactID}/root:/{name_folder}:/children?$select=name,eTag";
                ...
                if (response.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    ...
                    foreach (var item in list)
                    {
                        ...
                        {
                            var urlVideo = await CrmHelper.RetrieveImagesSharePoint<RetrieveMultipleApiResponse<GraphThumbnailsUrlModel>>($"{OrgConfig.SP_ContactID}/items/{item.id}/driveItem/thumbnails");
